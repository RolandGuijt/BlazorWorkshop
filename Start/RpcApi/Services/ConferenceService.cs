...
namespace RpcApi.Services
{
    public class ConferenceService: Conferences.ConferencesBase
    {
        ...
        public override Task<GetOneConferenceResponse> GetOne(GetOneConferenceRequest request, ServerCallContext context)
        {
            var result = new GetOneConferenceResponse();
            result.Conference = repo.GetOne(request.Id);
            return Task.FromResult(result);
        }
